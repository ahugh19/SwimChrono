...
interface VisStrokeCountTextLaneProps {
  layerList: LayerType[],
...
function VisStrokeCountTextLane(props: VisStrokeCountTextLaneProps) {
  ...
  useEffect(() => {
    ...
    if (_thisFrameLaneList && _thisFrameLaneList[laneIndex]) {
      ...
      setContentText(_thisFrameLaneList[laneIndex].strokeCount)
      setLaneData(_thisFrameLaneList[laneIndex])

      editableElementList.forEach((e) => {
        if (e.id === editableElementInVisConfig.strokeCountTextLane[0].id) {
          setTextStyle(e)
        }
      })
...
export default VisStrokeCountTextLane