...
interface VisWorldRecordTextLaneProps {
  layerList: LayerType[],
...
function VisWorldRecordTextLane(props: VisWorldRecordTextLaneProps) {
  ...
  const [contentText, setContentText] = useState<number | string>(0)
  ...
  useEffect(() => {
    ...
    if (_thisFrameLaneList && _thisFrameLaneList[laneIndex]) {
      setContentText(_thisFrameLaneList[laneIndex].world)
      setLaneData(_thisFrameLaneList[laneIndex])
      setTextStyle(editableElementList.find(
        (e) => e.id === editableElementInVisConfig.worldRecordTextLane[0].id
      ))
    }
...
export default VisWorldRecordTextLane