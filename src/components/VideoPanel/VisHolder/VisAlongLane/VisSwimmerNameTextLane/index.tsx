...
interface VisSwimmerNameTextLaneProps {
  layerList: LayerType[],
...
function VisSwimmerNameTextLane(props: VisSwimmerNameTextLaneProps) {
  ...
  useEffect(() => {
    ...
    if (_thisFrameLaneList && _thisFrameLaneList[laneIndex]) {
      setContentText(_thisFrameLaneList[laneIndex].name)
      setLaneData(_thisFrameLaneList[laneIndex])
      setTextStyle(editableElementList.find(
        (e) => e.id === editableElementInVisConfig.swimmerNameTextLane[0].id
      ))
    }
...
export default VisSwimmerNameTextLane