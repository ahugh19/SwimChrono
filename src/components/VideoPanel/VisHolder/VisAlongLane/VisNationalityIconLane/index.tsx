...
interface VisNationalityIconLaneLaneProps {
  layerList: LayerType[],
...
function VisNationalityIconLane(props: VisNationalityIconLaneLaneProps) {
  ...
  useEffect(() => {
    ...
    if (_thisFrameLaneList && _thisFrameLaneList[laneIndex]) {
      setContentText(_thisFrameLaneList[laneIndex].nationality)
      setLaneData(_thisFrameLaneList[laneIndex])
      setTextStyle(editableElementList.find(
        (e) => e.id === editableElementInVisConfig.nationalityIconLane[0].id
      ))
    }
  ...
  return (
    ...
    >
      <text
        ...
        fontSize={textStyle?.iconSize}
        visibility={visibility && textStyle?.visible ? "visible" : "hidden"}>
        {getFlagEmoji(contentText)}
      </text>
    </g>
  )
}

export default VisNationalityIconLane