...
interface VisAccelerationTextLaneProps {
  layerList: LayerType[],
...
function VisAccelerationTextLane(props: VisAccelerationTextLaneProps) {
  ...
  useEffect(() => {
    ...
    if (_thisFrameLaneList && _thisFrameLaneList[laneIndex]) {
      setContentText(_thisFrameLaneList[laneIndex].acceleration)
      setLaneData(_thisFrameLaneList[laneIndex])
      setTextStyle(editableElementList.find(
        (e) => e.id === editableElementInVisConfig.accelerationTextLane[0].id
      ))
    }
...
export default VisAccelerationTextLane